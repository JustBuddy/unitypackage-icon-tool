...
{
    internal class ImageDetection
    {
        private const int INT_SIZE = 4; // We only need to check the first four bytes of the file / byte array.
        ...
        /// <returns></returns>
        private static byte[] LoadFileRawBytes(string filePath)
        {
            ...
            using (FileStream stream = File.OpenRead(filePath))
            {
                ...
                {
                    if (stream.Read(buffer, 0, buffer.Length) < INT_SIZE) return null;
                }
            }
        ...
        {
            byte[] bmp = System.Text.Encoding.ASCII.GetBytes("BM");     // BMP
            byte[] gif = System.Text.Encoding.ASCII.GetBytes("GIF");    // GIF
            ...
            byte[] jpeg2 = new byte[] { 255, 216, 255, 225 };           // jpeg2 (canon)
